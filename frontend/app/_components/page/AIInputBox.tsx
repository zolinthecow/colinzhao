...
const AIInputBox: React.FC<Props> = ({ handleSubmit }) => {
  ...
  const { data } = trpc.bruh.useQuery({ text: 'what' });
  console.log(data);
  ...
  return (
    <div className="fixed bottom-6 w-full max-w-5xl flex flex-row">
      <textarea
        ref={textAreaRef}
        value={value}
      ...
      >
        {showTooltip && (
          <div className='absolute bottom-12 flex flex-col justify-center'>
            <div
              className='bg-zinc-900 text-input.foreground text-sm font-medium p-2 rounded-md whitespace-nowrap'
            >
              Send message
            </div>
