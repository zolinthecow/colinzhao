...
export default class ApiService implements IApiService {
  ...
  constructor() {
    if (process.env.NODE_ENV === 'development')
      this._apiUrl = `http://localhost:4000/`;
  ...
  async fetch<T extends keyof RestTypes>(
    input: T,
    init?: RequestInit | undefined,
  ...
  async generateStream<T extends keyof RestTypes>(
    input: T,
  ...
  private async *_getIterableStream<T extends keyof RestTypes>(
    body: ReadableStream<Uint8Array>,
  ): AsyncIterable<Array<RestTypes[T]['response']>> {
    ...
    while (true) {
      ...
      yield chunkArray.map(
        (c) => JSON.parse(c) as unknown as RestTypes[T]['response'],
      );
    }
  }
